...
reasult=0;
c = max(Y);
matrix=zeros(c,max(Label)); noise=zeros(c,1); s=size(Label,1);
for i = 1:s % test each example
    if Label(i)>0
        matrix(Y(i),Label(i))=matrix(Y(i),Label(i))+1;
    else
        noise(Y(i))=noise(Y(i))+1;
    end
end
...
if size(matrix,2)~=0
    [reasult,match]=fmean(matrix, noise);
    [posc,~]=find(match==1);
    for i=1:size(posc,1)
        indF(posc(i))=reasult(i);
    end
    reasult=sum(indF)/max(Y);    
end 
end

function [f1, match] = fmean( matrix, noise)
